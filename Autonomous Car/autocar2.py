...
while (True):
    ...
    if Traffic_Light == True:  # 파란불 찾기 모드일때
        ...
        img_mask1 = cv2.inRange(img_hsv, (174, 30, 30), (180, 255, 255))
        img_mask2 = cv2.inRange(img_hsv, (0, 30, 30), (8, 255, 255))
        img_mask3 = cv2.inRange(img_hsv, (169, 30, 30), (179, 255, 255))
    ...
    if customControlServer.OnOff == True:
        if t2 - t1 >= 0.2:
            ...
            lines=cv2.HoughLines(mask,1,np.pi/180,35)# 관심영역에서 선을그림
            ...
            listym2 = []
            point = 0
            customControlServer.velocity = 0.4
            ...
            if lines is not None:
                for i in range(0, len(lines)):
                    ...
                    try:
                        ...
                        if 0.05 < slope < 100:  # 1차적으로 쓸모없고 평균값 잡아먹는 말도안되는 기울기를 가진 직선을 1차적으로 필터링해줌
            ...
            if direction >= -3 and direction <= 3 and leftpoint ==0 and rightpoint == 0:
                '''
                if -5 < direction <= -2:
                    slcount += 1
                    if(slcount == 3):
                ...
                customControlServer.forward()
                print("go")
            elif direction <-3 or leftpoint != 0:
                lcount += 1
                if(lcount >= 3):
                    if leftpoint == 0:
                        customControlServer.stop()
                ...
                if(rcount >= 3):
                    ...
                    if rightpoint == 2:
                        if len(listxp1) == 0:
                            ...
                            time.sleep(0.1)
                            customControlServer.stop()
            print(customControlServer.velocity)
            print("left" + str(len(listxp1)) + "right" + str(len(listxm1)) )
            print("leftpoint" + str(leftpoint) + "rightpoint" + str(rightpoint) )
            t1 = time.time()
            t2 = time.time()
