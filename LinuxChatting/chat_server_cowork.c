...
void* ChattingRoom1(void* arg)
{
	...
	while (1)
	{
		...
		if ((rcv_len1 = recv(user_sck[0], msg, sizeof(msg), 0)) >= 0)
		{
			...
			{
				rcv_len1 = recv(user_sck[0], check, sizeof(check), 0);	//어떤 유저인지 정보 받기
...
void* Log_in_Thread(void* arg) { //로그인 쓰레드	
	char check1[512];	//id받는 배열
	char check2[512];	//pw받는 배열
	char IP[30];
	char PORT[10];
	int rcv_byte;
	int thread_fd;

	thread_fd = new_fd;		//전역변수의 소켓넘버를 지역변수로 넘겨줌
	...
	if (strcmp(check1, USER1_ID) == 0 && strcmp(check2, USER1_PW) == 0) { //log_in OK?
		if (user1_Flag == 1) {	//로그인 플래그가 1이면 이미 사용중인 계정
			...
			printf("USER1 IP:%s\n", P2P_IP1);
			printf("\n====Chatting Room====\n>>");
			SCK_NUM = SCK_NUM + 1;		//채팅 참여자 수 1명 추가

			pthread_create(&p_thread[1], NULL, ChattingRoom1, NULL);		//채팅룸1 스레드 생성
		}
	}
