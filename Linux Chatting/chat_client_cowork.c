...
void filelist(int p2p_sock)
{
	...
	char check[10] = { 0 };
	char check2[10] = { 0 };
	char s1[100] = "/home/st2016146036/finaldir";
	char s2[10]="/";
	char s3[10];
...
void Rcv_P2P(int p2p_fd)
{
	...
	printf("select File:");
	gets(number);
	send(p2p_fd, number, sizeof(number), 0);	
	rcv_len = recv(p2p_fd, txt_name, sizeof(txt_name), 0);
	send(p2p_fd, "ACK", 10, 0);
	rcv_len = recv(p2p_fd, size, sizeof(size), 0);	
	send(p2p_fd, "ACK", 10, 0);
	rcv_len = recv(p2p_fd, savetxt, sizeof(savetxt), 0); 	
	fp = fopen(txt_name, "w"); 		//텍스트 이름과 같은 내용의 파일을 write모드로 열기
...
void P2P_ServerSet()
{
	...
	int p2p_fd;
	p2p_sck = socket(AF_INET, SOCK_STREAM, 0); //Make socket!! 
	if (p2p_sck == -1) {
